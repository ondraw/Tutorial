...
# 100 + 1을 한 이유는 map의 내용을 보면 [:-1],[1:] 으로 마지막 전,1부터마지막 즉 100개를 리턴하게 되어 있다.
...
def main(_):
  ...
  checkpoint_dir = './work.training_checkpoints'
  ...
  if latest_ckpt:
      sampling_RNN_model.load_weights(latest_ckpt)
      sampling_RNN_model.build(tf.TensorShape([1, None]))
      sampling_RNN_model.summary()
      print("샘플링을 시작합니다!")
      print(generate_text(sampling_RNN_model, start_string=u'BUCKINGHAM What will he?'))
      exit()
